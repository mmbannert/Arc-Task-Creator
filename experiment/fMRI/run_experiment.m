...
for phaseIndex = 1:numel(block.phases)
    ...
    if phaseIndex == 1 || phaseIndex == 3
        ...
        trial = run_trial( ...
            window, windowRect, block, phase, phaseIndex, 0, ...
            phase.trial(1), textureCache, keys, config, scanStartTime);
    ...
    for trialIndex = 1:numel(phase.trials)
        trial = run_trial( ...
            window, windowRect, block, phase, phaseIndex, trialIndex, ...
            phase.trials(trialIndex), textureCache, keys, config, scanStartTime);
...
function trial = run_trial( ...
    window, windowRect, block, phase, phaseIndex, trialIndex, ...
    trialData, textureCache, keys, config, experimentStartTime)
